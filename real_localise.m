...
while strcmp(state,'lost') && iter<15
    ...
    for i = 1:num
        ...
        part_dists(i,:) = part_dists(i,end:-1:1);
        k = 1;
        for j = good_scans
    ...
    for i = 0:240
        ...
        if mod(i, 9) == 0
            ...
            new_part = datasample(part_pos_ang(1:10,:), 1);
            pos = new_part(1:2) + ((25-iter)/4)*[randn() randn()];
            ang = new_part(3) + 0.3*randn;
            part.setBotPos(pos);    
            part.setBotAng(ang);
...
%     similar_check = sum(abs(est_bot.ultraScan()- bot.dists))
    ...
    if convergence_score < 800 % || similar_check < 30
        ...
        if est_score^(-1/min(4,good_count)) > 20
            for i = 1:200
                part = particle(i);
                part.randomPose(0);
                iter  = 1;
